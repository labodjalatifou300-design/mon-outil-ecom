...
③ SCRIPTS VOIX-OFF : 3 scripts. CHAQUE SCRIPT DOIT CONTENIR ENTRE 130 ET 160 MOTS EXACTEMENT.
   COMPTE TES MOTS AVANT DE RÉPONDRE. Si un script fait moins de 130 mots → AJOUTE des phrases.
   Si plus de 160 mots → COUPE. \\n entre phrases courtes. Noms africains.
   RAPPEL : 130 mots minimum, 160 mots maximum — OBLIGATOIRE SANS EXCEPTION.
...
{{
  ...
  "scripts": [
    {{"angle":"Émotionnel","texte_complet":"ENTRE 130 ET 160 MOTS OBLIGATOIRES. Fluide. Noms africains. \\n entre phrases."}},
    {{"angle":"Bénéfice Direct","texte_complet":"ENTRE 130 ET 160 MOTS OBLIGATOIRES. Preuve sociale africaine."}},
    {{"angle":"Urgence & Statut","texte_complet":"ENTRE 130 ET 160 MOTS OBLIGATOIRES. Statut social et urgence."}}
  ],
  "angles_marketing": [
...
if page == "analyse":
    ...
    if st.session_state.get("analyzed") and st.session_state.get("result"):
        ...
        with tabs[4]:
            slabel("🎙️ Scripts Voix-Off (130–160 mots minimum)")
            scripts = data.get("scripts",[])
            for i, s in enumerate(scripts):
                txt = s.get("texte_complet", s.get("texte",""))
                wc  = len(txt.split())
                wc_col = "#3fb950" if 130<=wc<=160 else "#f0883e" if wc>160 else "#f85149"
