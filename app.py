...
function copyText(text, btnId) {
  navigator.clipboard.writeText(text).then(function() {
    ...
    if (btn) {
      btn.innerHTML = '✅ Copié !';
      setTimeout(function() { btn.innerHTML = '📋 Copier'; }, 2500);
    }
  });
}
</script>
...
def make_copy_btn(text, uid):
    ...
    return f'<button class="copy-btn" id="cpbtn_{uid}" onclick="copyText(\'{escaped}\',\'cpbtn_{uid}\')">📋 Copier</button>'

def section_header(title, copy_text, uid):
    return f'<div class="card-header"><span class="card-title">{title}</span>{make_copy_btn(copy_text, uid)}</div>'
...
def build_export(name, pa, pmin, pmax, data):
    sep = "="*60
    lines = [sep,"  ECOMASTER LABO PRO — PACK MARKETING",f"  Créé par LABO · {datetime.now().strftime('%d/%m/%Y %H:%M')}",
             f"  Produit : {name}",f"  Prix achat : {pa:,} FCFA | Vente : {pmin:,}–{pmax:,} FCFA",
...
with st.sidebar:
    st.markdown("""<div style="padding:0.5rem 0 0.75rem;">
      ...
      <p style="color:#333;font-size:0.68rem;margin:0;">by LABO · v5.0 Elite</p>
...
with col1:
    product_name = st.text_input("📦 Nom du Produit", value=st.session_state.get("active_product",""), placeholder="Ex: Bracelet Protection Ancestral")
    purchase_price = st.number_input("💰 Prix d'Achat (FCFA)", min_value=0, step=500, value=st.session_state.get("active_price",5000))
...
with col2:
    ...
    if uploaded_files:
        ...
        for i,f in enumerate(uploaded_files[:3]):
            with cols[i]: st.image(f, use_column_width=True)
...
with p1: st.markdown(f'<div class="price-box"><div class="label">💵 Prix Min</div><div class="value">{price_min:,}<span class="currency"> F</span></div></div>', unsafe_allow_html=True)
with p2: st.markdown(f'<div class="price-box"><div class="label">💵 Prix Max</div><div class="value">{price_max:,}<span class="currency"> F</span></div></div>', unsafe_allow_html=True)
with p3: st.markdown(f'<div class="price-box"><div class="label">📈 Marge</div><div class="value">8K–12K<span class="currency"> F</span></div></div>', unsafe_allow_html=True)
...
Adapte tout au contexte africain : Facebook, WhatsApp, statut social, confiance, utilité immédiate.

PRODUIT : {name}
PRIX ACHAT : {pa} FCFA | VENTE : {pmin}–{pmax} FCFA

Type produit :
- "wow" = effet visuel immédiat, pas besoin d'explication
- "probleme_solution" = résout un problème, nécessite explication

CONTRAINTES ABSOLUES :
1. SHOPIFY : EXACTEMENT 3 titres de page produit + EXACTEMENT 6 paragraphes. Chaque paragraphe = 1 titre en gras + 4 lignes de détails qui expliquent ce titre en profondeur.
2. FACEBOOK ADS : 3 variantes. Chaque variante = 1 titre accrocheur (ex: 🔥 PROMO FLASH, ⚠️ STOCK LIMITÉ, 💥 OFFRE EXCLUSIVE) + texte publicitaire MAX 5 lignes émotionnel et fun.
3. SCRIPTS VOIX-OFF : 3 scripts. Chaque script = bloc unique de EXACTEMENT 170 mots. Pas de titres internes, pas de "Hook:", "CTA:", juste le texte fluide à lire.
   - Si WOW : commence par effet choc visuel → problème rapide → solution → preuve (ex: "M. Kofi a testé...") → urgence
   - Si PROBLÈME-SOLUTION : frustration → solution → témoignage (ex: "Mme Aminata...") → comment ça marche → urgence

Réponds UNIQUEMENT avec JSON valide, sans texte avant ni après :

{{
  "score": 8,
  "score_justification": "analyse potentiel marché africain",
  "type_produit": "wow",
  "ameliorations": ["conseil 1", "conseil 2", "conseil 3"],
  "public_cible": "description très détaillée : âge, situation, revenus, habitudes africaines, douleurs, rêves",
  "peurs": ["peur précise 1", "peur 2", "peur 3"],
  "desirs": ["désir précis 1", "désir 2", "désir 3"],
  "mots_cles": ["mot1","mot2","mot3","mot4","mot5"],
  "offres": [
    {{"nom":"nom offre","description":"contenu précis","prix_suggere":"XXXX FCFA","argument":"pourquoi ça marche"}},
    {{"nom":"nom offre","description":"contenu","prix_suggere":"XXXX FCFA","argument":"argument"}},
    {{"nom":"nom offre","description":"contenu","prix_suggere":"XXXX FCFA","argument":"argument"}}
  ],
  "shopify": {{
    "titres": [
      {{"angle":"Bénéfice Principal","titre":"titre page produit 1"}},
      {{"angle":"Curiosité / Wow","titre":"titre page produit 2"}},
      {{"angle":"Urgence / FOMO","titre":"titre page produit 3"}}
    ],
    "paragraphes": [
      {{"titre":"Titre §1 en gras","texte":"Ligne 1 de détail.\\nLigne 2 de détail.\\nLigne 3 de détail.\\nLigne 4 de détail."}},
      {{"titre":"Titre §2 en gras","texte":"Ligne 1.\\nLigne 2.\\nLigne 3.\\nLigne 4."}},
      {{"titre":"Titre §3 en gras","texte":"Ligne 1.\\nLigne 2.\\nLigne 3.\\nLigne 4."}},
      {{"titre":"Titre §4 en gras","texte":"Ligne 1.\\nLigne 2.\\nLigne 3.\\nLigne 4."}},
      {{"titre":"Titre §5 en gras","texte":"Ligne 1.\\nLigne 2.\\nLigne 3.\\nLigne 4."}},
      {{"titre":"Titre §6 en gras","texte":"Ligne 1.\\nLigne 2.\\nLigne 3.\\nLigne 4."}}
    ]
  }},
  "facebook_ads": [
    {{"angle":"Émotionnel","accroche":"🔥 TITRE CHOC ICI","texte":"texte pub ligne 1\\nligne 2\\nligne 3\\nligne 4\\nligne 5 max — émotionnel avec emojis"}},
    {{"angle":"Preuve Sociale","accroche":"⭐ TITRE TÉMOIGNAGE ICI","texte":"texte pub max 5 lignes — témoignage africain authentique avec emojis"}},
    {{"angle":"Urgence","accroche":"⚠️ STOCK LIMITÉ — COMMANDE VITE","texte":"texte pub max 5 lignes — urgence et offre limitée avec emojis"}}
  ],
  "scripts": [
    {{"angle":"Script Émotionnel","texte_complet":"bloc de texte fluide EXACTEMENT 170 mots sans titres internes, prêt à lire en voix-off"}},
    {{"angle":"Script Bénéfice Direct","texte_complet":"bloc de texte fluide EXACTEMENT 170 mots sans titres internes, angle différent"}},
    {{"angle":"Script Urgence & Statut","texte_complet":"bloc de texte fluide EXACTEMENT 170 mots sans titres internes, joue sur statut social africain"}}
  ]
}}"""
    ...
    if not product_name.strip():
        ...
        with st.spinner("🧠 Analyse IA en cours... patiente 15–20 secondes ⏳"):
            try:
                ...
                st.session_state.update({"result":data,"analyzed":True,
...
if st.session_state.get("analyzed") and "result" in st.session_state:
    ...
    # Barre produit actif
    st.markdown(f"""<div style="background:#161b22;border:1px solid #2a3140;border-radius:10px;padding:0.6rem 1rem;margin-bottom:1rem;display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap;animation:fadeInDown 0.5s ease both;">
      <span style="color:#D90429;font-size:1.1rem;">📦</span>
      <div style="flex:1;min-width:150px;">
        <p style="color:#FFF;font-weight:700;margin:0;font-size:0.92rem;">{aname}</p>
        <p style="color:#444;font-size:0.72rem;margin:0;">Achat : {aprice:,} F · Vente : {apmin:,}–{apmax:,} FCFA</p>
      </div>
    ...
    with tab1:
        ...
        with c1:
            st.markdown(f"""<div class="score-wrap">
              ...
              {"<div class='wow-badge'>⚡ PRODUIT WOW</div>" if type_produit=="wow" else "<div class='ps-badge'>🎯 PROBLÈME-SOLUTION</div>"}
            </div>""", unsafe_allow_html=True)
        with c2:
            verdict = data.get("score_justification","")
            st.markdown(f"""<div class="result-card">
              {section_header("💡 Verdict IA", verdict, "verdict")}
        ...
        if score < 9:
            ...
            st.markdown(f"""<div class="amelioration-card">
              {section_header("⚠️ Comment Booster ce Produit ?", amelios_txt, "amelio")}
        ...
        # TABLEAU RENTABILITÉ
        def calc(v, pm, pa):
            ca = v*pm; prod=v*pa; lv=v*2000; cl=v*1000
            pub_f = {range(1,11):3000,range(11,21):5000,range(21,999):12000}
            pf=12000
            for r,val in pub_f.items():
                if v in r: pf=val; break
            bn = ca - prod - lv - cl - pf
            return ca,prod,lv,cl,pf,bn

        rows_html = ""
        volumes = sorted(set([5,10,20] + ([aventes] if aventes not in [5,10,20] else [])))
        for v in volumes:
            ca,prod,lv,cl,pf,bn = calc(v, apmoy, aprice)
            cls = "pos" if bn>0 else "neg"
            sign = "+" if bn>0 else ""
            hi = ' style="background:#0e0000 !important;"' if v==aventes else ""
            rows_html += f"""<tr{hi}>
              <td><b>{"🎯 " if v==aventes else ""}{v}/jour</b></td>
              <td>{ca:,.0f} F</td><td>{prod:,.0f} F</td>
              <td>{lv:,.0f} F</td><td>{cl:,.0f} F</td>
              <td><span class="pub-badge">{get_pub_budget(v)}</span></td>
              <td class="{cls}">{sign}{bn:,.0f} F</td>
            </tr>"""

        st.markdown(f"""<div class="result-card">
          <div class="card-header"><span class="card-title">💰 Tableau de Prévision Rentabilité</span></div>
          <div style="overflow-x:auto;">
          <table class="gains-table">
            <thead><tr><th>Volume</th><th>CA</th><th>Produit</th><th>Livraison</th><th>Closing</th><th>Pub</th><th>Bénéfice Net</th></tr></thead>
            <tbody>{rows_html}</tbody>
          </table></div>
          <p style="color:#333;font-size:0.68rem;margin-top:0.4rem;">* Livraison 2 000F · Closing 1 000F · Prix moyen {apmoy:,.0f} FCFA</p>
        </div>""", unsafe_allow_html=True)
        ...
        if mots:
            ...
            st.markdown(f"""<div class="result-card">
              {section_header("🔍 Mots-Clés", " · ".join(mots), "mots")}
              <div>{badges}</div>
            </div>""", unsafe_allow_html=True)

    # ── TAB 2 : OFFRES ───────────────────────────────────────────────────────
    with tab2:
        st.markdown("""<div style="background:#050f00;border:1px solid #336600;border-radius:12px;padding:0.75rem 1rem;margin-bottom:1rem;animation:fadeInDown 0.5s ease both;">
          <p style="color:#77dd22;font-weight:700;margin:0;font-size:0.85rem;">🎁 Offres pour booster tes ventes — Marché Africain</p>
        </div>""", unsafe_allow_html=True)
        for i,o in enumerate(data.get("offres",[])):
            ...
            st.markdown(f"""<div class="offre-card">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.35rem;">
                ...
                {make_copy_btn(offre_txt, f"offre_{i}")}
              </div>
    ...
    with tab3:
        ...
        st.markdown(f"""<div class="result-card">
          {section_header("🏷️ 3 Titres de Page Produit", titres_txt, "all_titres")}
        """, unsafe_allow_html=True)
        for i,t in enumerate(shopify.get("titres",[])):
            ...
            <div class="copy-area">{make_copy_btn(t.get("titre",""), f"titre_{i}")}</div>
            """, unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="result-card">
          {section_header("📝 Fiche Produit — 6 Paragraphes", all_paras_txt, "all_paras")}
        """, unsafe_allow_html=True)
        for j,para in enumerate(shopify.get("paragraphes",[])):
            ...
            st.markdown(f"""<div class="para-card">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.4rem;">
                ...
                {make_copy_btn(para_txt, f"para_{j}")}
              </div>
    ...
    # ── TAB 4 : FACEBOOK ADS ─────────────────────────────────────────────────
    with tab4:
        st.markdown("""<div style="background:#0e0000;border:1px solid #330000;border-radius:10px;padding:0.65rem 1rem;margin-bottom:1rem;animation:fadeInDown 0.5s ease both;">
          <p style="color:#888;font-size:0.78rem;margin:0;">💡 Chaque variante = 1 titre accrocheur + texte max 5 lignes · Copie dans Facebook Ads Manager</p>
        </div>""", unsafe_allow_html=True)
        for i,ad in enumerate(data.get("facebook_ads",[])):
            ...
            st.markdown(f"""<div class="ad-block">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                <span style="color:#D90429;font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;">📣 Variante {i+1} — {ad.get("angle","")}</span>
                {make_copy_btn(full, f"ad_{i}")}
              </div>
    ...
    with tab5:
        ...
        st.markdown(f"""<div style="background:#0e0000;border:1px solid #330000;border-radius:10px;padding:0.65rem 1rem;margin-bottom:1rem;animation:fadeInDown 0.5s ease both;">
          <p style="color:#D90429;font-weight:700;margin:0;font-size:0.82rem;">Type : {type_label} · ~170 mots · Bloc fluide prêt à lire</p>
        </div>""", unsafe_allow_html=True)
        for i,script in enumerate(data.get("scripts",[])):
            ...
            st.markdown(f"""<div class="script-block">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem;padding-bottom:0.4rem;border-bottom:1px solid #2a3140;">
                <span class="script-label" style="margin:0;border:none;padding:0;">🎙️ {script.get("angle","")}</span>
                <div style="display:flex;align-items:center;gap:0.5rem;">
                  <span style="color:#444;font-size:0.65rem;">{word_count} mots</span>
                  {make_copy_btn(texte_script, f"script_{i}")}
                </div>
              </div>
...
# ══════════════════════════════════════════════════════════════════
# CONFIGURATION CLÉ API — STREAMLIT CLOUD
# ══════════════════════════════════════════════════════════════════
# 1. Va sur share.streamlit.io
# 2. Clique "..." sur ton app → "Settings" → onglet "Secrets"
# 3. Colle exactement :
#    GROQ_API_KEY = "gsk_XXXXXXXXXXXXXXXXXXXXXXXX"
# 4. Clique "Save" puis "Reboot app"
# ══════════════════════════════════════════════════════════════════
